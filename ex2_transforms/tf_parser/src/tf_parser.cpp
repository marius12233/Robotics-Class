 ...
 * Date:    Sep 30, 2020
 *
 * This node subscribes to /tf and computes the position and
 * orientation of the end-effector in all the reference frames of the
 * robot, by using different representations for the orientation.
 * 
 * -------------------------------------------------------------------
 */
...
int main(int argc, char **argv)
{
    ...
    while(ros::ok())
    {
        ...
        try{
            transforms[0] = tf_buffer.lookupTransform("base_link", "link6",  ros::Time(0));
            transforms[1] = tf_buffer.lookupTransform("link1", "link6",  ros::Time(0));
            transforms[2] = tf_buffer.lookupTransform("link2", "link6",  ros::Time(0));
            transforms[3] = tf_buffer.lookupTransform("link3", "link6",  ros::Time(0));
            transforms[4] = tf_buffer.lookupTransform("link4", "link6",  ros::Time(0));
            transforms[5] = tf_buffer.lookupTransform("link5", "link6",  ros::Time(0));
        }
